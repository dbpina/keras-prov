...
class LearningRateScheduler(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if (self.model.capture_provenance and self.model.adaptation):
            # training_id = ""
            # if "TrainingModel" in self.model.transformations_task:
            #     training_id = self.model.transformations_task["TrainingModel"]
            # elif "trainingmodel" in self.model.transformations_task:
            #     training_id = self.model.transformations_task["trainingmodel"]     
                
            # adaptation_task_id = ""
            # if "Adaptation" in self.model.transformations_task:
            ...
            if (round(tmp_lr, 5) != round(lr, 5)):
                ...
                t1.begin()
                now = time.time()
                t1_output = DataSet("oAdaptation", [Element([lr, now, epoch, self.adaptation_id])])
